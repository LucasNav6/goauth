...
func main() {
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		conn, err := pgx.Connect(ctx, "postgresql://neondb_owner:npg_lQ3yXHuGEcV6@ep-steep-paper-ac4c15ul-pooler.sa-east-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require")
		if err != nil {
		...
		goauthConfig := goauth.SetupConfiguration("passwordExample", queries, ctx, goauth_models.PasswordPolicy{
			MinLength:           8,
		...
		res, err := emailAndPasswordProvider.SignIn(goauthConfig, &goauth_models.Credentials{
			Email:     "user@example.com",
			Password:  &password,
			UserAgent: func() *string { ua := r.UserAgent(); return &ua }(),
			IP:        func() *string { ip := r.RemoteAddr; return &ip }(),
		})
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
